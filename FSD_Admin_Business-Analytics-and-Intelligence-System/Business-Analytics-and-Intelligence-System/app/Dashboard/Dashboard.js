document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCharts(projects) {
        ...
        updateChartData(projectStatusChart, statusLabels, statusData, 
    ...
    function updateStatusPieChart(projects) {
        ...
        const relevantStatuses = Object.keys(statusCounts).filter(status => statusCounts[status] > 0);
        const relevantCounts = relevantStatuses.map(status => statusCounts[status]);

        // Update data for status pie chart
        updatePieChartData(statusPieChart, relevantStatuses, relevantCounts, [
            'rgba(255, 99, 132, 0.2)',
            'rgba(54, 162, 235, 0.2)',
            'rgba(255, 206, 86, 0.2)',
            'rgba(75, 192, 192, 0.2)',
